...
import fpinscala.iomonad.{IO, Monadic}
...
input (`Await`) or signaling termination via `Halt`.
  ...
  object Process {
    ...
    def dropWhile[I](f: I => Boolean): Process[I, I] = await[I, I] { i =>
      if (f(i)) dropWhile(f)
      else emit(i, id)
    }
    ...
    def mean: Process[Double, Double] = {
      def go(sum: Double, count: Double): Process[Double,Double] =
        await((d: Double) => emit((sum+d) / (count+1), go(sum+d,count+1)), Halt())
      go(0.0, 0.0)
    }
    ...
    def sum2: Process[Double, Double] =
      loop(0.0)((d: Double, acc) => (acc, acc + d))

    def count3[I]: Process[I, Int] =
      loop(0)((_: I, count) => (count, count + 1))
    ...
    def zip[I, O1, O2](p1: Process[I, O1], p2: Process[I, O2]): Process[I, (O1, O2)] = (p1, p2) match {
      ...
      case (Await(recv1), _) =>
        Await((oa: Option[I]) => zip(recv1(oa), feed(oa)(p2)))
      case (_, Await(recv2)) =>
        Await((oa: Option[I]) => zip(feed(oa)(p1), recv2(oa)))
    }
    ...
    def mean2: Process[Double, Double] = zip(sum, count) |> lift(t => t._1/t._2)
    ...
    def exists[I](f: I => Boolean): Process[I, Boolean] = Await{
      case Some(i) => if (f(i)) emit(true, exists(f)) else emit(false, exists(f))
      case None => Halt()
    }

    /* Awaits then emits a single value, then halts. */
    def echo[I]: Process[I, I] = await[I,I](i => emit[I, I](i))
    ...
    def terminated[I]: Process[I, Option[I]] =
      await((i: I) => emit(Some(i), terminated[I]), emit[I, Option[I]](None))
